...
class Auth extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(['Auth_model', 'Mh_model']);

        $this->auth = $this->Auth_model;
        $this->user = $this->Mh_model;
    }
    ...
    public function login()
    {
        if (isset($_POST['submit']))
        {
            ...
            if ($verify == 1) {
                ...
                $this->user->logged($this->session->userdata('npm'));
                ...
                $this->session->set_userdata(['error' => 'Error!! Email atau password salah']);
                redirect('auth/login');
            }
    ...
    public function logout()
    {
        ...
        $this->db->update('mhs', ['is_logged_in' => 0]);
